...
public class ItemRouter {
    ...
    public RouterFunction<ServerResponse> itemRoute(ItemHandler handler) {
        return RouterFunctions.route(
                GET("/functional/items").and(accept(APPLICATION_JSON)), handler::getAllItems)
                .andRoute(GET("/functional/items/{id}"), handler::getItem)
                .andRoute(POST("/functional/items").and(accept(APPLICATION_JSON)), handler::createItems)
                .andRoute(DELETE("/functional/items/{id}"), handler::deleteItem)
                .andRoute(PUT("/functional/items/{id}"), handler::updateItem)
                .andRoute(GET("/functional/exception").and(accept(APPLICATION_JSON)), handler::itemException)
                .andRoute(GET("/functional/itemCapped").and(accept(APPLICATION_STREAM_JSON)), handler::itemCappedStream
                );
    }
}
